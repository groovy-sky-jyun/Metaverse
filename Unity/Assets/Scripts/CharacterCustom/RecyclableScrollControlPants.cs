...
public class RecyclableScrollControlPants : MonoBehaviour, IRecyclableScrollRectDataSource
{
    ...
    //Dummy data List
    private List<PantsItemDictionary> pantsItemList = new List<PantsItemDictionary>();
    ...
    {
        if (pantsItemList != null) pantsItemList.Clear();
        for (int i = 0; i < _dataLength; i++)
        {
            PantsItemDictionary obj = item.pantsItemList[i];
            if (obj.have)
            {
                pantsItemList.Add(obj);
            }
        }
    }
    ...
    public void SetCell(ICell cell, int index)
    {
        ...
        var item = cell as PantsItemSetting;
        item.ConfigureCell(pantsItemList[index], index);
    }
