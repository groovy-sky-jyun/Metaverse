...
public class RecyclableScrollControlWall : MonoBehaviour, IRecyclableScrollRectDataSource
{
    ...
    //Dummy data List
    private List<WallItemDictionary> wallItemList = new List<WallItemDictionary>();
    ...
    {
        if (wallItemList != null) wallItemList.Clear();
        Debug.Log(_dataLength);
        for (int i = 0; i < _dataLength; i++)
        {
            WallItemDictionary obj = item.wallItemList[i];
            if (!obj.have)
            {
                wallItemList.Add(obj);
                Debug.Log(obj.name);
            }
        }
    }
    ...
    public void SetCell(ICell cell, int index)
    {
        ...
        var item = cell as WallItemSetting;
        item.ConfigureCell(wallItemList[index], index);
    }
