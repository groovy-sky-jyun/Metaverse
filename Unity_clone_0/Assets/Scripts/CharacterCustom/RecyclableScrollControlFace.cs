...
public class RecyclableScrollControlFace : MonoBehaviour, IRecyclableScrollRectDataSource
{
    ...
    //Dummy data List
    private List<FaceItemDictionary> faceItemList = new List<FaceItemDictionary>();
    ...
    {
        if (faceItemList != null) faceItemList.Clear();
        for (int i = 0; i < _dataLength; i++)
        {
            FaceItemDictionary obj = item.faceItemList[i];
            if (obj.have)
            {
                faceItemList.Add(obj);
            }
        }
    }
    ...
    public void SetCell(ICell cell, int index)
    {
        ...
        var item = cell as FaceItemSetting;
        item.ConfigureCell(faceItemList[index], index);
    }
