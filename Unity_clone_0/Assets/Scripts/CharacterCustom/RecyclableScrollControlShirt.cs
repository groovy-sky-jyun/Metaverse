...
public class RecyclableScrollControlShirt : MonoBehaviour, IRecyclableScrollRectDataSource
{
    ...
    //Dummy data List
    private List<ShirtItemDictionary> shirtItemList = new List<ShirtItemDictionary>();
    ...
    {
        if (shirtItemList != null) shirtItemList.Clear();
        for (int i = 0; i < _dataLength; i++)
        {
            ShirtItemDictionary obj = item.shirtItemList[i];
            if (obj.have)
            {
                shirtItemList.Add(obj);
            }
        }
    }
    ...
    public void SetCell(ICell cell, int index)
    {
        ...
        var item = cell as ShirtItemSetting;
        item.ConfigureCell(shirtItemList[index], index);
    }
